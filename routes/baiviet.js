...
router.post("/them", async (req, res) => {
    if (req.session.MaNguoiDung) {
        ...
        req.session.success = "Đăng bài viết thành cong va dang cho kiem duyet";
...
router.post("/sua/:id", async (req, res) => {
    ...
    req.session.success = "Sua bai viet thanh cong va dang cho kiem duyet";
    res.redirect("/success");
});
// GET: Xóa bài viết
...
router.get("/cuatoi", async (req, res) => {
    if (req.session.MaNguoiDung) {
        ...
        var bv = await BaiViet.find({ TaiKhoan: id }).populate("ChuDe").populate("TaiKhoan").exec();
        res.render("baiviet_cuatoi", {
            title: "Bài viết cua toi",
            baiviet: bv,
        });
    } else {
        res.redirect("/dangnhap");
